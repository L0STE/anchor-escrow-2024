...
#[tokio::test]
async fn successful_token_take() {
    ...
    let mint_b = create_mint(&mut banks_client, &payer, Some(mint_b))
        .await;
    ...
    get_token(&mut banks_client, vault).await.expect("failed to get token");
    
    let taker_mint_b = create_and_mint_to_token_account(
        &mut banks_client,
        mint_b,
        &payer,
        taker.pubkey(),
    ...
    get_token(&mut banks_client, taker_mint_b).await.expect("failed to get token");
